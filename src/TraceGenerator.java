...
public class TraceGenerator {
	...
	private static String transitionsName = "envModel_small.txt";
			...
			while ((str = br.readLine()) != null) {
				...
				if (line[0].startsWith("Q")) {
					...
					if (line[line.length - 1].endsWith(",")
							|| line[line.length - 1].endsWith(".")) {
						m1 = Integer.parseInt(line[3].substring(1,
								line[3].length() - 2));
						...
						|| line[line.length - 1].endsWith(".")) {
					m1 = Integer.parseInt(line[4].substring(1,
							line[4].length() - 2));
	 ...
	 *
	 * @param random
	 *            When two post-condition are observed, true. When only one
	 *            post-condition is observed, false.
	 *
	 */
	public static void Generate(boolean random) {
		...
		try {
			...
			while (count <= traceSize + 1) {
				...
				} else if (transitionsName.startsWith("envModel_large")) {
					// GenerateLarge(random, s, r);
					if (random == false) {
				...
				} else if (transitionsName.startsWith("envModel_small")) {
					if (random == false) {
						// When only one post-condition is observed
						if (pre.equals("putdown")) {
							pw.println(s.map.get(s.keys.get(0)));
							stateNum = s.keys.get(0);
						} else if (pre.equals("pickup")) {
							pw.println(s.map.get(s.keys.get(1)));
							stateNum = s.keys.get(1);
						} else {
							pw.println(s.map.get(s.keys.get(r)));
							stateNum = s.keys.get(r);
						}
						pre = s.map.get(stateNum);
					} else if (prepre.startsWith("arrive")) {
